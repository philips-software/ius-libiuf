...
#include <include/iusDrivingScheme.h>
...
)
{
    ...
    baseDrivingScheme->numTransmitSources = iusSourceListGetSize( transmitSources );
    baseDrivingScheme->numTransmitPulses = iusPulseDictGetSize(transmitPulses);
...
//
//
//int iusDrivingSchemeSet3DSourceLocation
//(
...
//}
//
//int iusDrivingSchemeSet2DSourceLocation
//(
//    IusDrivingScheme *drivingScheme,
...
)
{
    ...
    if( apodization < 0.0f || apodization > 1.0f )
        return IUS_ERR_VALUE;
    iusApodizationListSetApodization(drivingScheme->pApodizations, apodizationIndex, elementIndex, apodization);
    return IUS_E_OK;
}
//
...
)
{
    ...
    if( elementIndex >= drivingScheme->numElements || elementIndex < 0 )
        return NAN;
    return iusApodizationListGetApodization(drivingScheme->pApodizations, pulseIndex, elementIndex);
}
