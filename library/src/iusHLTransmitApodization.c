...
)
{
	...
	status |= iusHdf5WriteFloat(group_id, IUS_INPUTFILE_PATH_TRANSMITAPODIZATION_APODIZATION, transmitApodization->apodization, transmitApodization->numElements, 1);
...
)
{
	...
	transmitApodization = iusHLTransmitApodizationCreate(numElements);
	for (idx = 0; idx < numElements; idx++)
	{
		status |= iusHLTransmitApodizationSetElement(transmitApodization, idx, apodization[idx]);
	}
	if (status != 0) return NULL;
