...
TEST(IusFrameList, testIusCompareFrameList)
{
  ...
  iuf_t notherObj = iusFrameCreate(3,4,0.11f);
...
TEST(IusFrameList, testIusSerialization)
{
  ...
  status = iusFrameListSave(frameList, pFrameListPath, handle);
  TEST_ASSERT_EQUAL(IUS_E_OK, status);
  ...
  iufl_t savedFrameList = iusFrameListLoad(handle, pFrameListPath);
