...
TEST(IufIqPatternListDict, testIufCreateDeletePatternListDict)
{
    ...
    TEST_ASSERT(iufIqPatternListDictDelete(obj) == IUF_E_OK);
    TEST_ASSERT(iufIqPatternListDictDelete(notherObj) == IUF_E_OK);
    TEST_ASSERT(iufIqPatternListDictDelete(NULL) != IUF_E_OK);
    TEST_ASSERT(iufIqPatternListDictDeepDelete(NULL) != IUF_E_OK);
}
...
TEST(IufIqPatternListDict, testIufComparePatternListDict)
{
	...
	TEST_ASSERT_EQUAL(IUF_TRUE, equal);
    equal = iufIqPatternListDictCompare(notherDict, dict);
    TEST_ASSERT_EQUAL(IUF_TRUE, equal);
...

{
	...
	// save
    status = iufIqPatternListDictSave(dict, H5I_INVALID_HID);
    TEST_ASSERT(status != IUF_E_OK);

    hid_t handle = H5Fcreate(filename, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
	TEST_ASSERT(handle > 0);
    status = iufIqPatternListDictSave(NULL, handle);
    TEST_ASSERT(status != IUF_E_OK);
	...
	// read back
    iuiqpald_t savedObj = iufIqPatternListDictLoad(H5I_INVALID_HID);
    TEST_ASSERT_EQUAL(NULL,savedObj);

	handle = H5Fopen(filename, H5F_ACC_RDONLY, H5P_DEFAULT);
	savedObj = iufIqPatternListDictLoad(handle);
...
{
	RUN_TEST_CASE(IufIqPatternListDict, testIufCreateDeletePatternListDict);
