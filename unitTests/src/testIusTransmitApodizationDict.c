...
TEST(IusTransmitApodizationDict, testIusTransmitApodizationDictSerialization)
{
	...
	status = iusTransmitApodizationDictSave(transmitApodizationDict, TransmitApodizationDictPath, handle);
	H5Fclose(handle);
	TEST_ASSERT_EQUAL(IUS_E_OK, status);

	// read back
	handle = H5Fopen(filename, H5F_ACC_RDONLY, H5P_DEFAULT);
	iutad_t savedDict = iusTransmitApodizationDictLoad(handle, TransmitApodizationDictPath);
