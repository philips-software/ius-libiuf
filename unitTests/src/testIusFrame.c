...
{
    iuf_t obj = iusHLFrameCreate(0,1);
    iuf_t notherObj = iusHLFrameCreate(3,8);
    ...
    // invalid params
    obj = iusHLFrameCreate(-1,1);
    TEST_ASSERT(obj == IUF_INVALID);
    obj = iusHLFrameCreate(1,-2.0);
    TEST_ASSERT(obj == IUF_INVALID);
}

TEST(IusFrame, testIusFrameDelete)
{
    iuf_t obj = iusHLFrameCreate(1,2);
    TEST_ASSERT(obj != IUF_INVALID);
...
TEST(IusFrame, testIusFrameCompare)
{
    IUS_BOOL equal;
    iuf_t obj = iusHLFrameCreate(1,2);
    iuf_t notherObj = iusHLFrameCreate(1,2);
    iuf_t differentObj = iusHLFrameCreate(3,4);
    TEST_ASSERT(obj != IUF_INVALID);
    TEST_ASSERT(notherObj != IUF_INVALID);
...
TEST(IusFrame, testIusFrameSetGet)
{
    iuf_t obj = iusHLFrameCreate(1,2);
    TEST_ASSERT(obj != IUF_INVALID);
    iuf_t notherObj = iusHLFrameCreate(101,202);
    TEST_ASSERT(notherObj != IUF_INVALID);
...
TEST(IusFrame, testIusSerialization)
{
    ...
    // create and save
    iuf_t obj = iusHLFrameCreate(patternListIndex,dataIndex);
